...
export function resolveAgentCommand(
  ...
  // Windows: CURSOR_AGENT_NODE+SCRIPT bypasses cmd.exe's ~8191 char limit.
  // Unix: same vars let the host override the agent's bundled node (avoids
  // segfaults with --list-models on some macOS installs).
  if (loaded.agentNode && loaded.agentScript) {
    ...
    const invoker = platform === "win32" ? "agent.cmd" : "agent";
    return {
      command: loaded.agentNode,
      args: [loaded.agentScript, ...args],
      env: { ...env, CURSOR_INVOKED_AS: invoker },
      agentScriptPath,
    };
  }
